...
int main(){
    int n; gli(n); vi v; gli(v); vvi v3(3); int ind=0;
    //L(3) v3.PB(vi{}); unnecessary
    for (int i:v){ ind++;
        v3[i-1].PB(ind);
    }
    int len = std::min({v3[0].size(), v3[1].size(), v3[2].size()});
    if (len==0) prt(0);
    else{ prtl(len); LR(i,len) prtl(v3[0][i],v3[1][i],v3[2][i] ); }
}
